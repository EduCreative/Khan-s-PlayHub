...
import { Send, MessageSquare, Zap, X, Wifi, WifiOff, Minimize2 } from 'lucide-react';
...
export const TactileQuickChat: React.FC<TactileQuickChatProps> = ({ userProfile, isDarkMode }) => {
  ...
  // Listen to Firestore Realtime Messages
  useEffect(() => {
    const q = query(
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setChats(prev => {
        // If chat panel is closed, increase unread count for truly new messages
      ...
      // Play alert sound for new messages written by others
  ...
  const sendQuickChat = async (text: string, type: 'preset' | 'emoji' | 'custom') => {
    ...
    // Check auth status
    if (!auth.currentUser) {
      alert("Please Sign In using Google first to broadcast chat signals!");
      return;
    }
    ...
    try {
      ...
      console.error("Local caching message dispatch:", error);
      // Fallback local addition if network restriction is active
          ...
          >
            ...
            <div className="p-5 border-b border-indigo-500/10 flex items-center justify-between bg-gradient-to-r from-indigo-950/30 via-transparent to-transparent">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-black uppercase italic text-white tracking-wide flex items-center gap-1.5">
                    Live Chat Console
                  </h3>
                  {/* Status Indicator Tag */}
                  <span className={`inline-flex items-center gap-1 text-[8px] font-extrabold uppercase tracking-widest px-2 py-0.5 rounded-full border mt-0.5 ${
                    isOffline 
                      ? 'bg-amber-500/10 border-amber-500/20 text-amber-400' 
                      : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'
                  }`}>
                    <span className={`w-1 h-1 rounded-full ${isOffline ? 'bg-amber-400 animate-pulse' : 'bg-emerald-400 animate-ping'}`} />
                    {isOffline ? '💾 Local Cache Active' : '🌐 Multi-Link: Online'}
                  </span>
                </div>
              </div>
                ...
                {chats.length === 0 ? (
                  ...
                  chats.map((chat) => (
                    ...
                    >
                      ...
                      <div className="w-7 h-7 rounded-lg bg-indigo-500/10 border border-white/5 flex items-center justify-center text-indigo-400 shrink-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-baseline gap-1.5">
                          <span className="text-[11px] font-extrabold text-slate-300 italic truncate">
                            {chat.senderUsername}
                          </span>
              ...
              <div className="pt-2 border-t border-indigo-500/10">
                ...
                >
                  <input
                    ...
                    disabled={isSending}
                    className="flex-1 px-3 py-2 rounded-xl bg-slate-900 border border-white/10 text-[11px] font-medium text-white placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 transition-all"
                  />
                  <button
                    type="submit"
